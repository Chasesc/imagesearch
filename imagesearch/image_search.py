...
def load_image_from_path(path):
    return imread(path)[:, :, :3] # just keep rgb

class ImageSearch(object):
    def __init__(self):
        self.inference = image_inference.ImageInference()
        self._images = {}

    def add_images(self, *images):
        for image in images:
            image = str(image) # ensure we have a string and not a pathlib path
            self._images[image] = load_image_from_path(image)
            print(self._images[image].shape, image)
